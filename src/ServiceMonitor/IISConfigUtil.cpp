...
std::string IISConfigUtil::GetRunningPools()
{
	std::string pStrCmd;
	pStrCmd.append(Convert2String(m_pstrSysDirPath));
	pStrCmd.append("\\inetsrv\\appcmd.exe list apppools /state:stopped");

	ProcessUtil processUtil;

	string result;
	processUtil.RunCommand(pStrCmd, result);
	return result;
}


std::vector<std::pair<std::string, bool>> IISConfigUtil::ParseRunningPools(std::string input)
{
	vector<std::pair<std::string, bool>> kvp;
	...
	for (vector<string>::iterator it = result.begin(); it != result.end(); ++it) {
		...
		result2.clear();
		Tokenize(Trim(*it," "), " ", result2);

		pair.first = Trim(result2[1], "\"");

		/*if (StringToLower(result2[2]).find("state:started")) {
			pair.second = true;
			pair.first = Trim(result2[1], "\"");
		}
		else if (StringToLower(result2[2]).find("state:stopped"))
		{
			pair.second = false;
		}*/
		if (StringToLower(result2[2]).find("state:stopped") != string::npos)
		{
			pair.second = false;
			kvp.push_back(pair);
		}
...
string IISConfigUtil::StartPools(std::string pool)
{
	std::string pStrCmd;
	pStrCmd.append(Convert2String(m_pstrSysDirPath));
	pStrCmd.append("\\inetsrv\\appcmd.exe start apppool /apppool.name:");
	pStrCmd.append(pool);
